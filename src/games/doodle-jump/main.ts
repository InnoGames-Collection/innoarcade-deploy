...
window.addEventListener('keyup', (e) => {
  if (e.key === 'ArrowLeft' || e.key === 'a' || e.key === 'ArrowRight' || e.key === 'd') game.releaseDir();
});
...
const loop = new GameLoop(
  ...
  () => {
    ...
    $('#heightVal').textContent = String(game.score);
  },
);
