  ...
  if (!pageId) {
    stack.innerHTML = `
      ...
      <nav class="acct-menu-list">
        ${accountRowHtml('aIdentity', 'identity', t('identity'))}

        ${acctUser ? accountRowHtml('aInvite', 'invite', t('invite')) : ''}
      </nav>
      
      <div class="acct-nav-sec">SUPPORT</div>
      <nav class="acct-menu-list">
        ${accountRowHtml('aHelp', 'support', t('help'))}
        ${accountRowHtml('aFaq', 'faq', t('faq'))}
      </nav>

      <div class="acct-nav-sec">SERVICE</div>
      <nav class="acct-menu-list">
        ${accountRowHtml('aPricing', 'pricing', t('pricing'))}
        ${accountRowHtml('aSubscription', 'subscription', t('subscription'))}
      </nav>

      <div class="acct-nav-sec">INFORMATION</div>
      <nav class="acct-menu-list">
        ${accountRowHtml('aAbout', 'about', t('about'))}
        ${accountRowHtml('aTerms', 'terms', t('terms'))}
      </nav>

      <div class="acct-nav-sec">SETTINGS</div>
      <nav class="acct-menu-list">
        ${accountRowHtml('aSettings', 'settings', t('settings'))}
      </nav>
      ${acctUser ? `
      <div class="acct-nav-sec">SESSION</div>
      <nav class="acct-menu-list">
        ${accountRowHtml('aLogout', 'logout', t('signOut'), true, true)}
      </nav>` : ''}
    `;
    wireAccount();
    ...
    stack.innerHTML = `
      ...
      : `
        ...
        <nav class="acct-menu-list" style="margin-top: 1.2rem;">
          ${accountRowHtml('subIdSignOut', 'logout', t('signOut'), true, true)}
        </nav>`}
    `;
    ...
    stack.innerHTML = `
      ...
      <div class="acct-menu-list">
        ${settingsRowHtml('setLang', 'language', 'Language', `<span style="color:#8892b0; font-size:0.9rem;">${langMap[curLang] || curLang}</span> ${chevHtml}`)}
        ${settingsRowHtml('setSound', 'sound', 'Sound', toggleHtml(sfxOn))}
        ${settingsRowHtml('setNotif', 'notifications', 'Notifications', chevHtml)}
        ${settingsRowHtml('setFaq', 'faq', 'FAQ', chevHtml)}
        ${settingsRowHtml('setTerms', 'terms', 'Terms & Conditions', chevHtml)}
        ${settingsRowHtml('setPrivacy', 'privacy', 'Privacy Policy', chevHtml)}
        ${settingsRowHtml('setAppInfo', 'appinfo', 'App Version', `<span style="color:#8892b0; font-size:0.9rem;">1.0.0</span>`)}
      </div>
    `;
    ...
    stack.innerHTML = `
      ...
      <div class="acct-menu-list">
        ${settingsRowHtml('ntfTourn', 'tournament', 'Tournament', toggleHtml(d.notifTournament))}
        ${settingsRowHtml('ntfChall', 'challenge', 'Daily Challenge', toggleHtml(d.notifChallenge))}
        ${settingsRowHtml('ntfReward', 'rewards', 'Rewards', toggleHtml(d.notifRewards))}
      </div>
    `;
